...
contract PopulateSystemLayoutStructs {
  ...
  function populateSystemLayoutStructs(bytes32 randomish) external view returns (Structs.System memory system, Structs.Planet[] memory) {
    ...
    (system.nGas, system.nRocky, system.nHabitable) = getPlanetCategoriesCounts(plCategory);
    ...
    for (uint i=0; i<plRadii.length;) {
      ...
      (planets[i].resources, planets[i].resourceAbundance) = getPlanetResources(randomish, i, plCategory[i]);
  ...
  function getPlanetTurbScale(bytes32 randomish, uint256 index, uint8 plCategory) public pure returns (uint16 turbScale) {
    ...
    if (plCategory == 0) {
      ...
      turbScale = uint16(bytes2(0x0000) | ( bytes2(randomish[index+18]) >> 8 )) % 21 + 5;
    }
    // Rocky planet 
    else if (plCategory == 1) {
      // turbScale = uint16(bytes2(0x0000) | ( bytes2(randomish[index+18]) >> 8 )) % 21 + 20;
      turbScale = uint16(bytes2(0x0000) | ( bytes2(randomish[index+18]) >> 8 )) % 61 + 20;
    }
    // Water world 
    else {
  ...
  // function getPlanetResources(bytes32 randomish, uint256 index, uint8 plCategory) public pure returns (uint8[3] memory, uint8[3] memory) {
  function getPlanetResources(bytes32 randomish, uint256 index, uint8 plCategory) public view returns (uint8[3] memory, uint8[3] memory) {
    ...
    if (plCategory == 0) {
      ...
      for (uint i=0; i<3;) {
        ...
        if (picker <= 49) {
          prelimResources[i] = 4; // Aluminum
        }
        else if (picker > 49 && picker <= 94) {
          prelimResources[i] = 5; // Iron
        }
        else if (picker > 94 && picker <= 131) {
          prelimResources[i] = 6; // Nickel
        }
        else if (picker > 131 && picker <= 164) {
          prelimResources[i] = 7; // Copper
        }
        else if (picker > 164 && picker <= 191) {
          prelimResources[i] = 8; // Silicon
        }
        else if (picker > 191 && picker <= 206) {
          prelimResources[i] = 9; // Gold
        }
        else if (picker > 206 && picker <= 219) {
          prelimResources[i] = 10; // Titanium
        }
        else if (picker > 219 && picker <= 230) {
          prelimResources[i] = 11; // Lithium
        }
        else if (picker > 230 && picker <= 239) {
          prelimResources[i] = 12; // Cobalt
        }
        else if (picker > 239 && picker <= 246) {
          prelimResources[i] = 13; // Platinum
        }
        else if (picker > 246 && picker <= 251) {
          prelimResources[i] = 14; // Chromium
        }
        else if (picker > 251 && picker <= 254) {
          prelimResources[i] = 15; // Terbium
        }
        else if (picker == 255) {
          prelimResources[i] = 16; // Selenium
        }
